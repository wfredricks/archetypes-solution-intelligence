 ...
 *
 *   Lift origin: byte-for-byte copy of `contract-loader/src/backends/select.ts`
 *   (Phase 1e) with the provenance JSDoc updated for the Phase 2.5
 *   lift origin.
 *
 *   Ownership: asi-local. The single entry point that callers of
 *   `runCompletenessAgent` / `runBookendAuditAgent` go through to
 *   obtain a `Backend`.
 */
