 ...
 *   under BUILD-PHASE-2_5-PLAN.md §2.5.2.
 *
 *   Lift origin: pattern lifted from `contract-loader/src/backends/neo4j-backend.ts`
 *   (Phase 1e). Adapted for the agents package — adds two native
 *   helpers (`countOutgoingRels`, `countIncomingRels`) that contract-
 *   loader does not need but completeness-agent's three aggregation
 *   query sites require.
 *
 *   Ownership: asi-local. Wraps the prior `neo4j-driver` plumbing
 *   from `completeness-agent.ts` + `bookend-audit-agent.ts` (Phase
 *   1b) behind the `Backend` interface so callers do not import
 *   `neo4j-driver` directly. Behavior is byte-for-byte identical to
 *   the pre-Phase-2.5 path: same cypher, same `session.run()` shape,
 ...
 * // class is a thin shim — every `query()` is one `session.run()`
 * // plus a `records.map(r => r.toObject())`.
 *
...
export class Neo4jBackend implements Backend {
   ...
   * connects to `options.graphUrl` (defaulting to
   * `bolt://localhost:7689`) and takes ownership.
   */
  ...
  /**
   * Expose the underlying driver. Some tests reach in and run their
   * own cypher to seed/wipe the namespace; rather than grow the
   * Backend interface with `seed`/`wipe` we expose the driver. Same
   * choice as contract-loader's Neo4jBackend (Phase 1e).
   */
  getDriver(): Driver {
    return this.driver;
  }
  ...
  /**
   * Native primitives. The Neo4j path issues one tiny cypher per call
   * so the agent code path looks identical in shape across backends.
   */
  native = {
    findNodes: async (
      label: string,
    ...
    countOutgoingRels: async (
      ...
      // Why: agent code that has a node id already (obtained via
      // findNodes) needs a single-call count instead of materializing
      // the whole edge list. On Neo4j this is the same shape the
      // OPTIONAL MATCH + WITH + count(...) cypher used to emit, just
      // tucked behind a tiny method to keep the per-backend code
      // paths uniform.
      const session = this.driver.session();
      try {
        const typeFilter = types && types.length > 0 ? `:${types.map((t) => `\`${t}\``).join('|')}` : '';
        const res = await session.run(
          `MATCH (n) WHERE id(n) = $id MATCH (n)-[r${typeFilter}]->() RETURN count(r) AS c`,
          { id: parseIntegerId(nodeId) },
        );
        if (res.records.length === 0) return 0;
        const c = res.records[0].get('c');
        return typeof c === 'object' && c !== null && 'toNumber' in c
          ? (c as { toNumber: () => number }).toNumber()
          : Number(c ?? 0);
      } finally {
        await session.close();
      }
    ...
    countIncomingRels: async (
      ...
      try {
        const typeFilter = types && types.length > 0 ? `:${types.map((t) => `\`${t}\``).join('|')}` : '';
        const res = await session.run(
          `MATCH (n) WHERE id(n) = $id MATCH ()-[r${typeFilter}]->(n) RETURN count(r) AS c`,
          { id: parseIntegerId(nodeId) },
        );
        if (res.records.length === 0) return 0;
        const c = res.records[0].get('c');
 ...
 * matched node/relationship variable `varName` is supported.
 */
...
/**
 * Parse a stringified Neo4j internal id back to a number.
 *
 * // Why: the Backend interface uses string ids so PolyGraph's
 * // uuid-style ids interoperate. Neo4j's id() is an integer; we
 * // round-trip via string for the interface but reach back to a
 * // number when issuing cypher.
 */
function parseIntegerId(s: string): number {
  const n = Number(s);
  if (Number.isFinite(n)) return n;
  throw new Error(`Neo4jBackend: expected numeric id, got "${s}"`);
}
