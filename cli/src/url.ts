/**
 * Derived from archetypes/solution-intel/reference-impl/cli/src/url.ts
 ...
 * Modifications:
 *   - @adopt:project-config-path ANSWERED → `.asi/config.yaml`
 *   - @adopt:default-endpoint-env-var ANSWERED → `ASI_URL`
 *   - All references to `.si/config.yaml` updated to `.asi/config.yaml`
 *   - process.env.SI_URL → process.env.ASI_URL
 *   - Docstrings refer to ASI/I (was SI/I)
 */

/**
 * URL + project-config resolution for the `asi` CLI.
 *
 * // Why: Three legal places to specify the ASI/I base URL — `--url` flag,
 * // `ASI_URL` env var, or a `.asi/config.yaml` discovered by walking up from
 ...
 * // from `.asi/config.yaml` — `si.graphUrl`, `si.studioUrl`, etc. —
 * // without duplicating the walk-up logic. The new top-level surface is
 * // `resolveProjectConfig()`, which returns the full `.asi/config.yaml`
 ...
 *
 * `.asi/config.yaml` shape (v0.1; the YAML key remains `si:` as the
 * neutral substrate-config namespace inherited from solution-intel):
 *
 *   si:
...
export interface UrlResolution {
  ...
  /** Path to the `.asi/config.yaml` that supplied the URL (when applicable). */
  configPath?: string;
}

/**
 * Parsed `.asi/config.yaml` record.
 *
...
export interface ProjectConfig {
  /** Path to the `.asi/config.yaml` file when discovered via walk-up. */
  path?: string;
 ...
 * `.asi/config.yaml` and return the discovered path + parsed `si:` block.
 *
 ...
 * // first `.asi/config.yaml` encountered while walking toward the filesystem
 * // root is returned. We stop at the root rather than ascending into HOME so
 * // a stray `~/.asi/config.yaml` does not accidentally apply to every command.
 *
 ...
 * `.asi/config.yaml` with a non-empty `si:` block is found.
 */
async function readProjectConfig(
  ...
  for (let i = 0; i < 50; i++) {
    ...
    // ANSWER:  .asi/config.yaml (matches @adopt:namespace = asi). The
    //          inner YAML key is preserved as `si:` (it's a substrate-
    //          neutral key inherited from solution-intel; only the
    //          containing directory carries the adopter namespace).
    // Format: relative path under each candidate directory
    const candidate = path.join(cur, '.asi', 'config.yaml');
...
/**
 * Walk up looking for `.asi/config.yaml` and return `{ path, url }`.
 *
 ...
 * // `.asi/config.yaml` record so it can pull graphUrl, studioUrl, etc.
 ...
 *
 * Behavior:
 * - Walks up looking for `.asi/config.yaml` (via {@link readProjectConfig}).
...
export async function resolveProjectConfig(
  ...
  // 1. Walk up to find .asi/config.yaml; parse the full si: block.
  ...
  // Default: SI_URL
  // ANSWER:  ASI_URL (matches @adopt:namespace = asi; ASI_ prefix is the
  //          namespace's env-var prefix)
  // Format: SCREAMING_SNAKE_CASE matching ^[A-Z][A-Z0-9_]{2,31}$
  const envUrl = process.env.ASI_URL?.trim();
