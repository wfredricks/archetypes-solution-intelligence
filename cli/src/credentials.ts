/**
 * Derived from archetypes/solution-intel/reference-impl/cli/src/credentials.ts
 ...
 * Modifications:
 *   - @adopt:credentials-dir ANSWERED → `~/.asi` (was `~/.si`)
 *   - Docstrings refer to ASI/I and `asi login`/`asi grant`/`asi revoke`
 *   - File-layout doc-block reflects `~/.asi/credentials`
 */

/**
 * Credentials store for the `asi` CLI.
 *
 * // Why: After `asi login` succeeds, the user holds a long-form ASI/I token
 * // they shouldn't have to retype for every subsequent command. We persist
 * // it on disk under `~/.asi/credentials` keyed by ASI/I base URL so a single
 ...
 * // canonical form, a user could `asi login` against one form and find their
 ...
 *
 * // Why: First-time login on a fresh workstation has no `~/.asi/`. We create
