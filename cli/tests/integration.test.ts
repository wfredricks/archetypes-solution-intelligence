/**
 * Derived from archetypes/solution-intel/reference-impl/cli/tests/integration.test.ts
 ...
 * Modifications:
 *   - Test labels `si login` / `si grant` / `si revoke` → asi equivalents
 *   - `~/.si/credentials` reference → `~/.asi/credentials`
 *   - SI_AUDIT_PATH reference → ASI_AUDIT_PATH
 */

/**
 * End-to-end integration test (adopter).
 *
 * // Why: This is the load-bearing test. We boot the real
 * // ASI/I server on a random port, drive the three CLI commands
...
describe('CLI integration (Stage 2b)', () => {
  ...
  it('asi login (alice) writes ~/.asi/credentials mode 0600', async () => {
  ...
  it('asi login (root) succeeds and stores a second entry under the same URL key', async () => {
  ...
  it('asi grant (root grants alice Operator on p-integration)', async () => {
  ...
  it('asi revoke removes the grant and emits an audit event', async () => {
  ...
  it('audit ledger contains both granted and revoked events for the grant', async () => {
    // Why: The integration test sets ASI_AUDIT_PATH so the chainblocks
    ...
    const grantedHits = events.filter(
      (e) => e.type === 'asi.role.granted',
    );
    const revokedHits = events.filter(
      (e) => e.type === 'asi.role.revoked',
    );
