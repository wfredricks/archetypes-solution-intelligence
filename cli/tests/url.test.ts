/**
 * Derived from archetypes/solution-intel/reference-impl/cli/tests/url.test.ts
 ...
 * Modifications:
 *   - SI_URL env-var references → ASI_URL
 *   - Fixture config path `.si/config.yaml` → `.asi/config.yaml`
 *   - tmpdir prefix 'si-cli-url-' → 'asi-cli-url-'
 *   - The on-disk YAML key remains `si:` (substrate-neutral; only the
 *     containing dotfile dir carries the namespace)
 */

/**
 * Tests for src/url.ts.
 *
 * // Why: Precedence bugs in URL resolution would let the wrong ASI/I
...
beforeEach(async () => {
  originalEnv = process.env.ASI_URL;
  delete process.env.ASI_URL;
  originalCwd = process.cwd();
  tmp = await fs.mkdtemp(path.join(os.tmpdir(), 'asi-cli-url-'));
});

afterEach(async () => {
  if (originalEnv === undefined) delete process.env.ASI_URL;
  else process.env.ASI_URL = originalEnv;
...
async function writeConfig(dir: string, url: string | null): Promise<string> {
  const cfgDir = path.join(dir, '.asi');
...
/**
 * Write a `.asi/config.yaml` with a custom YAML body.
 *
...
describe('resolveUrl precedence', () => {
  it('returns flag when provided, ignoring env + config', async () => {
    process.env.ASI_URL = 'http://from-env:1';
  ...
  it('uses config when flag + env are absent', async () => {
    ...
    const realCfg = path.join(realRoot, '.asi', 'config.yaml');
    const out = await resolveUrl();
...
describe('findProjectConfig walk-up', () => {
  ...
  it('surfaces a clear error when YAML is malformed', async () => {
    const cfgDir = path.join(tmp, '.asi');
    await fs.mkdir(cfgDir, { recursive: true });
...
describe('resolveProjectConfig (Stage 2c)', () => {
  it('returns the full si: block when the config has multiple keys', async () => {
    ...
    const realRoot = process.cwd();
    expect(out.config.path).toBe(path.join(realRoot, '.asi', 'config.yaml'));
  });
