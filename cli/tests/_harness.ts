/**
 * Derived from archetypes/solution-intel/reference-impl/cli/tests/_harness.ts
 ...
 * Modifications:
 *   - All SI_* env-var names → ASI_* (snapshot keys + spawn env)
 *   - tmpdir prefixes 'si-cli-int-*' → 'asi-cli-int-*'
 *   - Server log prefix '[si-identity]' → '[asi-identity]'
 *   - Docstrings refer to ASI/I and `~/.asi/credentials`
 */

/**
 * Shared integration-test harness for the `asi` CLI.
 *
 ...
 *   - Snapshots the relevant env vars (HOME, ASI_GRANTS_PATH,
 *     ASI_AUDIT_PATH, ASI_DEV_CODE, ASI_URL) so {@link HarnessHandle.stop}
 *     can restore them exactly.
 *   - Builds `../identity/dist/server.js` if it is missing.
 *   - Picks a free TCP port via Node's net module.
 *   - Spawns the ASI/I server with `ASI_DEV_CODE=123456` (overridable),
...
interface EnvSnapshot {
  ...
  ASI_GRANTS_PATH: string | undefined;
  ASI_AUDIT_PATH: string | undefined;
  ASI_DEV_CODE: string | undefined;
  ASI_URL: string | undefined;
}
...
export interface HarnessOptions {
  /**
   * Deterministic verification code the ASI/I server should accept.
...
export async function bootIdentityHarness(
  ...
  const envSnapshot: EnvSnapshot = {
    ...
    ASI_GRANTS_PATH: process.env.ASI_GRANTS_PATH,
    ASI_AUDIT_PATH: process.env.ASI_AUDIT_PATH,
    ASI_DEV_CODE: process.env.ASI_DEV_CODE,
    ASI_URL: process.env.ASI_URL,
  };

  // Tmp HOME so ~/.asi/credentials lives somewhere disposable.
  const tmpHome = await fs.mkdtemp(path.join(os.tmpdir(), 'asi-cli-int-home-'));
  process.env.HOME = tmpHome;
  // Tmp data dir for the server's grants + audit jsonl.
  const tmpData = await fs.mkdtemp(path.join(os.tmpdir(), 'asi-cli-int-data-'));
  ...
  // Why: We do not carry ASI_URL into the server process; the CLI uses
  // --url. Unset it so resolveUrl does not pick up a stray dev value.
  delete process.env.ASI_URL;

  const serverProc: ChildProcess = spawn('node', [identityDist], {
    cwd: identityRoot,
    env: {
      ...process.env,
      ASI_PORT: String(serverPort),
      ASI_DEV_CODE: devCode,
      ASI_ALLOWED_DOMAINS: '*',
      ASI_PROJECT_ID: 'p-integration',
      ASI_GRANTS_PATH: grantsPath,
      ASI_AUDIT_PATH: auditPath,
    },
  ...
  serverProc.stderr?.on('data', (buf: Buffer) => {
    process.stderr.write(`[asi-identity] ${buf.toString('utf-8')}`);
  });
