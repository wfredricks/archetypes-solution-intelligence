/**
 * Derived from archetypes/solution-intel/reference-impl/cli/tests/credentials.test.ts
 ...
 * Modifications:
 *   - tmpdir prefix 'si-cli-creds-' → 'asi-cli-creds-'
 *   - credentialsDir / credentialsPath assertions: '.si' → '.asi'
 */
...
beforeEach(async () => {
  ...
  tmpHome = await fs.mkdtemp(path.join(os.tmpdir(), 'asi-cli-creds-'));
...
describe('credentialsDir / credentialsPath', () => {
  it('respects HOME', () => {
    expect(credentialsDir()).toBe(path.join(tmpHome, '.asi'));
    expect(credentialsPath()).toBe(path.join(tmpHome, '.asi', 'credentials'));
  });
});
